...
def app_principal():
    ...
    def crear_pdf(historial, nombre, peso, objetivo):
        ...
        pdf.cell(0, 10, txt=f"CLIENTE: {nombre} | FECHA: {datetime.date.today()}", ln=1, align='L')
        ...
        for mensaje in historial:
            if mensaje["role"] == "model":
                pdf.multi_cell(0, 7, txt=mensaje["content"].replace("**", "").replace("*", "-"))
                pdf.ln(5)
    ...
    with st.sidebar:
        ...
        if "history" in st.session_state and len(st.session_state.history) > 1 and st.session_state.get('is_premium'):
             ...
             st.download_button("📥 Descargar Rutina PDF", pdf_bytes, "Rutina_Zynte.pdf", use_container_width=True)
    ...
    with tab_train:
        ...
        if col_b2.button("🧘 Estiramientos", use_container_width=True):
            prompt_ejecutar = "Dame una rutina de estiramientos para mejorar la postura y espalda."
